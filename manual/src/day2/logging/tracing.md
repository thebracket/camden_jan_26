...
> This example is found in `code/day2/tracing_json`.

For machine-readable logs, enable the JSON formatter on the subscriber. This pairs well with an environment filter and optional span timing events.
...
Set `RUST_LOG` to control verbosity (e.g., `RUST_LOG=info cargo run`). Each event is emitted as a single JSON object suitable for ingestion by log processors.
