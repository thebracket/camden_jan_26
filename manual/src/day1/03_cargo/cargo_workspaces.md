...
Have a look in your `hello_l3h` directory after you've run/compiled the program. There's a `target` directory. This is where build artifacts, including your executable file. Even with just the one dependency, there can be a lot in there:
...
Let's start by cleaning up. In your `hello_l3h` directory, run:
...
Now make sure you are still in your `hello_l3h` directory. Let's add a new project. This time, we'll make a library.
